defmodule TlcElixirWeb.GridComponents do
  ...
  def program_cell(assigns) do
    ~H"""
    <div class={"flex-1 flex flex-col relative #{if @col_idx == @program_length - 1, do: "border-r", else: ""} border-gray-600 #{if @current_cycle == @cycle && !@editing, do: "z-10 rounded ring-2 ring-purple-600 ring-offset-1 ring-offset-gray-900", else: ""}"}>
      <!-- Header cell -->
  ...
  def switch_cell(assigns) do
    ~H"""
        <div class={"p-1 h-8 flex items-center justify-between border-r border-b border-gray-600 #{if @is_switch_point, do: "bg-gray-400", else: ""} #{if @editing, do: "cursor-pointer", else: ""}"}
             phx-value-cycle={@cycle}
             data-switch-cycle={@cycle}
             phx-click={if @editing, do: "set_switch_point"}
             title={if @editing, do: "Click to set switch point", else: ""}>
          <%= if @is_switch_point do %>
            <!-- Switch point is indicated by background color only. No text marker. -->
    ...
    <div class={"p-1 h-8 flex items-center justify-center border-r border-b border-gray-600 #{if @is_start_of_skip || @is_within_skip, do: "bg-gray-400", else: ""} relative"}
      ...
      <% else %>
        <%= if @current_cell_has_invalid_skip do %>
          <.invalid_transition_indicator title={@skip_error_tooltip} class="cursor-pointer" />
        <% else %>
          <span class="opacity-0">0</span>
        <% end %>
      <% end %>
    ...
    <%= for {_group, i} <- Enum.with_index(@program.groups) do %>
      ...
      >
        <%= if has_invalid_transition do %>
          <.invalid_transition_indicator title={error_tooltip} />
        <% end %>
        <span class="text-gray-200 select-none"><%= signal %></span>
      </div>
    <% end %>
  ...
  attr :class, :string, default: "invalid-transition-indicator"
  def invalid_transition_indicator(assigns) do
    ~H"""
    <div class={@class} title={@title} data-tooltip-content={@title}>
