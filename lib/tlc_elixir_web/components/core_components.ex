defmodule TlcElixirWeb.CoreComponents do
  ...
  def interval_buttons(assigns) do
    ...
    assigns =
      assign(assigns, intervals: intervals, selected_interval: selected, paused: assigns[:paused])
        ...
        <div class="flex items-center gap-1 justify-start mt-1">
          <form id="manual-step-form-header" phx-submit="step" class="flex items-center gap-1">
            <.pill
              ...
              type="button"
              class={
                if(@paused, do: "bg-purple-700", else: "bg-gray-700 hover:bg-gray-600 text-gray-300")
              }
              aria-pressed={@paused}
              title="Pause"
            >
            ...
            <div class={"flex items-center gap-1 " <> if(not @paused, do: "opacity-50", else: "") }>
              <input
                ...
                value="1"
                min="1"
                class={"w-16 px-3 py-1 rounded bg-gray-700 text-gray-300 border border-gray-600 " <> if(not @paused, do: "bg-gray-800 text-gray-500", else: "") }
                disabled={!@paused}
              />
              <.pill
                tag="button"
                ...
                class={
                  if(not @paused,
                    do: "bg-gray-800 text-gray-500 cursor-not-allowed",
  ...
  def pill(assigns) do
    ...
    <%= if @tag == "button" do %>
      <button
        {@rest}
        class={"inline-flex items-center gap-1 px-3 py-1 rounded text-sm transition-colors text-white " <> @class}
      >
        {render_slot(@inner_block)}
      </button>
    <% else %>
