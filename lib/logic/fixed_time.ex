defmodule Tlc.Logic.FixedTime do
    ...
    case Map.get(logic.program.waits || %{}, logic.cycle_time) do
      ...
      duration when is_integer(duration) and duration > 0 ->
        if logic.waited < duration do
          ...
          logic
          |> Map.update!(:offset_adjust, fn adj ->
            Integer.mod(adj - logic.unix_delta, logic.program.length)
          end)
  ...
  def apply_skips(%{target_distance: dist} = logic) when dist > 0 do
    case Map.get(logic.program.skips || %{}, logic.cycle_time) do
      ...
      duration when is_integer(duration) and duration > 0 ->
        logic
        |> Map.update!(:offset_adjust, fn adj ->
          Integer.mod(adj + duration, logic.program.length)
        end)
        |> update_offset()
        |> compute_cycle_time()
        |> find_target_distance()
  ...
  def update_offset(logic) do
    %{
      logic
      | offset: Integer.mod(logic.program.offset + logic.offset_adjust, logic.program.length)
    }
  end
  ...
  def sync(logic, target_cycle_time) do
    %{
      ...
      | offset_adjust:
          Integer.mod(
            target_cycle_time - logic.unix_time - logic.program.offset,
            logic.program.length
          )
    }
    |> update_offset
    |> compute_cycle_time
  ...
  def fault(logic, fault_program) do
    %{logic | program: fault_program, target_program: nil, mode: :fault}
    |> update_base_time()
  ...
  def recover(logic, halt_program) do
    %{logic | program: halt_program, target_program: nil, mode: :halt}
    |> update_base_time()
