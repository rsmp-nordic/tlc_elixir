defmodule Tlc.Server.SwitchController do
  ...
  def switch_program(tlc, program_name) do
    ...
    if program do
      ...
      if updated_logic != tlc.logic do
        ...
        resumed_logic =
          if Tlc.Logic.Protocol.mode(tlc.logic) == :halt do
  ...
  def switch_program_immediate(tlc, program_name) do
    ...
    if program do
      immediate_result =
        Tlc.Logic.Protocol.switch_immediate(tlc.logic, program, tlc.virtual_unix_time)
  ...
  def maybe_switch_to_target_program(%{target_program: target_program, logic: logic} = tlc) do
    # Do not attempt to switch when in fault mode
      ...
      |> case do
        ...
        :continue ->
          if not Tlc.Logic.Protocol.at_switch_point?(logic) do
            ...
            new_logic =
              Tlc.Logic.Protocol.switch_immediate(logic, target_program, tlc.virtual_unix_time)

            new_logic =
              if new_logic == logic do
                current_state = Tlc.Logic.Protocol.current_states(logic)

                Tlc.Program.Factory.create_matching(
                  target_program,
                  current_state,
                  tlc.virtual_unix_time
                ) ||
