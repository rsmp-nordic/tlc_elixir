defmodule Tlc.Program.SwitchValidator do
  ...
  2. Switch compatibility validation - ensures states match at switch points

  When switching between programs, the states at switch points must match to ensure
  a safe transition. This module checks all possible program pairs and reports any
  mismatches.
  ...
  def validate_switch(source, target) do
    ...
    # Check each combination of leave points from source with enter points of target
    for {source_point, source_state} <- source_switch_points,
        {target_point, target_state} <- target_switch_points,
        source_state != target_state do
      %{
  ...
  Returns true if any valid switch point combination exists.
  """
