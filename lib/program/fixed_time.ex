defmodule Tlc.Program.FixedTime do
  ...
  @derive {Jason.Encoder,
           only: [:name, :length, :offset, :groups, :states, :skips, :waits, :switch, :halt]}
  defstruct name: "",
            length: 0,
            offset: 0,
            groups: [],
  ...
  defp validate_groups(_),
    do: {:error, "Program must have at least one signal group defined as a list"}
  ...
  defp validate_states(%{states: states, length: length, groups: groups})
    ...
    state_errors =
      Enum.reduce_while(states, [], fn {time, state}, acc ->
        cond do
          ...
          String.length(state) != group_count ->
            {:halt,
             [
               "State strings must have the same length as the number of signal groups (#{group_count})"
             ]}
  ...
  defp validate_states(%{states: states}) when is_map(states),
    do: {:error, "Program must have at least one state defined"}
  ...
  defp validate_skips(program = %{skips: skips, length: length}) when is_map(skips) do
    skip_errors =
      Enum.reduce_while(skips, [], fn {time, count}, acc ->
        cond do
          not is_integer(time) or time < 0 or time >= length ->
          ...
          true ->
            ...
            if start_state != end_state do
              {:halt,
               [
                 "Skip start and end states must be the same at time #{time}, end_time #{end_time}"
               ]}
            else
              {:cont, acc}
            end
  ...
  defp validate_waits(%{waits: waits, length: length}) when is_map(waits) do
    wait_errors =
      Enum.reduce_while(waits, [], fn {time, duration}, acc ->
        cond do
          not is_integer(time) or time < 0 or time >= length ->
  ...
  defp validate_switch(_),
    do: {:error, "Switch must be an integer between 0 and program length - 1"}
        ...
        for cycle <- 0..(program.length - 1),
          ...
          if current_signal != next_signal do
            ...
            if next_signal not in valid_next_signals do
              ...
              {{next_cycle, group_idx},
               "Invalid transition from '#{current_signal}' to '#{next_signal}'. Valid transitions from '#{current_signal}' are: #{Enum.join(valid_next_signals, ", ")}"}
            end
          end
        end
    ...
    compact_states =
      case time_points do
        ...
        [first | rest] ->
          ...
          rest_states =
            Enum.reduce(rest, {%{first => first_state}, first_state}, fn time,
                                                                         {acc_map, prev_state} ->
            ...
            # Extract just the map from the tuple
            |> elem(0)
  ...
  def add_group(program, group_name) do
    ...
    updated_states =
      Map.new(program.states, fn {cycle, state} ->
        {cycle, state <> "D"}
      end)
  ...
  def remove_group(program, group_idx) do
    if group_idx < length(program.groups) do
      ...
      updated_states =
        Map.new(program.states, fn {cycle, state} ->
          {cycle,
           String.slice(state, 0, group_idx) <>
             String.slice(state, group_idx + 1, String.length(state))}
        end)
  ...
  """
  def validate_state_transition(current_states, new_states)
      when byte_size(current_states) == byte_size(new_states) do
    ...
    |> Enum.reduce_while(:ok, fn idx, _acc ->
      ...
      if current_signal != new_signal do
        ...
        if new_signal in valid_next_signals do
          ...
          error_msg =
            "Invalid transition from '#{current_signal}' to '#{new_signal}' for group #{idx}. " <>
